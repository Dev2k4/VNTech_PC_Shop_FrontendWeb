...
const ProductCard = ({ product }) => {
    ...
    return (
        <Box 
            ...
            position="relative"
            h="420px" // Tăng chiều cao thẻ để chứa ảnh to hơn
            display="flex"
            flexDirection="column"
            role="group" // Để xử lý hover effect cho ảnh con
        >
            <Link to={`/products/${id}`} style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                
                {/* Phần Ảnh - TO HƠN, ÍT PADDING HƠN */}
                <Flex 
                    flex="1" 
                    align="center" 
                    justify="center" 
                    p={4} // Giảm padding từ 6 xuống 4 hoặc 0 nếu muốn tràn viền
                    bg="black" 
                    position="relative"
                >
                    <Image 
