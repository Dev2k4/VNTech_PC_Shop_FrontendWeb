...
import { 
    ...
    Image, Text, IconButton, Button, Flex, Divider, useToast
...
const CartPage = () => {
    ...
    const handleUpdateQuantity = async (itemId, newQuantity) => {
        ...
        try {
            ...
            fetchCart(); // Load lại data để tính lại tổng tiền
        } catch (error) {
    ...
    if (loading) return <Box p={10} color="white" textAlign="center">Đang tải giỏ hàng...</Box>;

    if (!cart || cart.cartItems.length === 0) {
        return (
            <Box minH="60vh" bg="apple.bg" pt={20} textAlign="center" color="white">
    ...
    return (
        <Box bg="apple.bg" minH="100vh" py={10} color="white">
            <Container maxW="container.lg">
                <Heading mb={8} borderBottom="1px solid" borderColor="whiteAlpha.200" pb={4}>
                ...
                <Flex direction={{ base: "column", lg: "row" }} gap={10}>
                    ...
                    <Box flex="2">
                        {cart.cartItems.map((item) => (
                            <Flex key={item.id} py={6} borderBottom="1px solid" borderColor="whiteAlpha.200" align="center">
                                <Image 
                                ...
                                <Box flex="1">
                                    ...
                                    <Text color="gray.400" fontSize="sm" mb={2}>Giao hàng miễn phí</Text>
                                    <Flex align="center" gap={4}>
                                        <Flex align="center" border="1px solid" borderColor="gray.600" borderRadius="full" px={2} py={1}>
                                            <IconButton 
                    ...
                    {/* Tổng tiền */}
                    <Box flex="1">
                        <Box bg="apple.card" p={6} borderRadius="2xl" position="sticky" top="100px">
                            <Heading size="md" mb={6}>Tóm tắt đơn hàng</Heading>
                            <Flex justify="space-between" mb={3}>
                                <Text color="gray.400">Tạm tính</Text>
                                <Text>{formatCurrency(cart.totalPrice)}</Text>
                            </Flex>
                            <Flex justify="space-between" mb={6}>
                                <Text color="gray.400">Vận chuyển</Text>
