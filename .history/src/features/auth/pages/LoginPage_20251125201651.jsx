...
const LoginPage = () => {
  ...
  const handleLogin = async (formData) => {
    ...
    try {
      ...
      const { accessToken, refreshToken, role, id, username } = data; // Thêm lấy username

      localStorage.setItem("accessToken", accessToken);
      localStorage.setItem("refreshToken", refreshToken);
      localStorage.setItem("role", role);
      localStorage.setItem("userId", id);
      // localStorage.setItem("username", username); // Nếu API có trả về tên thì lưu luôn

      // --- THÊM DÒNG NÀY ĐỂ HEADER CẬP NHẬT ---
