...
const RegisterPage = () => {
    ...
    // Lưu thông tin form để dùng khi gửi lại OTP
    const [savedFormData, setSavedFormData] = useState(null);

    // Xử lý Đăng ký (Bước 1 -> 2)
    const handleRegister = async (formData) => {
        setIsLoading(true);
        try {
            await AuthService.register(formData);
            
            // Lưu lại data để dùng cho nút Resend
    ...
    // Xử lý Gửi lại OTP (Gọi lại API Register với data cũ)
    const handleResendOtp = async () => {
        if (!savedFormData) return;
        
        // Không bật loading toàn màn hình để tránh lag UI đếm ngược, chỉ hiện toast
        try {
