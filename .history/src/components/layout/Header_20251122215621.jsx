...
import { Box, Container, Flex, Link as ChakraLink, Icon, IconButton, Badge } from '@chakra-ui/react';
import { Link, useLocation } from 'react-router-dom';
import { FaApple, FaSearch, FaShoppingBag, FaBars } from 'react-icons/fa';
import { useCart } from '../../context/CartContext'; // Nếu chưa làm CartContext thì tạm bỏ dòng này
...
const Header = () => {
    ...
    // Mock cart item count (hoặc lấy từ Context thực tế)
    const cartCount = 2; 

    return (
        <Box 
            as="header" 
            bg="rgba(22, 22, 23, 0.8)" // Màu đen mờ đặc trưng của Apple
        ...
        >
            <Container maxW="container.xl" h="44px">
                <Flex h="100%" align="center" justify="space-between">
                    ...
                    {/* Desktop Menu Items */}
                    <Flex display={{ base: 'none', lg: 'flex' }} gap={8} align="center" flex={1} justify="center">
                        {MENU_ITEMS.map((item) => (
                            <ChakraLink 
                                key={item.label}
                                as={Link} 
                                to={item.path}
                                fontSize="12px" // Font nhỏ chuẩn Apple
                                fontWeight="400"
                                color="#e8e8ed"
