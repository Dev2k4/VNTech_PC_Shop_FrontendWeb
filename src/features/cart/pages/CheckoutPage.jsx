...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async () => {
    if (!selectedAddressId) {
      toast({
        title: "Vui lòng chọn địa chỉ nhận hàng",
        status: "warning",
        duration: 2000, // Nhanh hơn
        position: "top",
      });
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        if (paymentMethod === "VNPAY" && res.data.paymentUrl) {
          ...
          toast({
            ...
            status: "success",
            duration: 2000, // 2 giây là đủ đọc
            position: "top",
          });
      ...
      toast({
        ...
        status: "error",
        duration: 3000, // Lỗi quan trọng để lâu hơn xíu (3s)
      });
    } finally {
      setLoading(false);
    }
  ...
  if (!cart)
    return (
      <Box p={10} textAlign="center" color={textColor}>
        Đang tải...
      </Box>
    );
        ...
        <Grid templateColumns={{ base: "1fr", lg: "2fr 1fr" }} gap={10}>
          <VStack align="stretch" spacing={8}>
            ...
            >
              ...
              {addresses.length === 0 ? (
                <Text color="red.500">
                  Bạn chưa có địa chỉ nào. Hãy thêm mới.
                </Text>
              ) : (
                <RadioGroup
                  onChange={(val) => setSelectedAddressId(parseInt(val))}
              ...
              >
                {cart.cartItems.map((item) => (
                  <Flex key={item.id} justify="space-between">
                    <Box>
                      <Text fontWeight="medium" noOfLines={1} color={textColor}>
          ...
          <ModalContent bg={boxBg} color={textColor}>
            ...
            <ModalBody pb={6}>
              <VStack spacing={4}>
                ...
                <Button colorScheme="blue" w="full" onClick={handleAddAddress}>
                  Lưu địa chỉ
                </Button>
              </VStack>
            </ModalBody>
