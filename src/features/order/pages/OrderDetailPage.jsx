...
import {
    Box, Container, Grid, VStack, Heading, Flex, Badge, Button, useToast, Spinner, 
    useColorModeValue, useDisclosure, Text, Divider, Step, StepIndicator, StepStatus, 
    StepIcon, StepTitle, StepSeparator, Stepper, Icon
...
const OrderTracker = ({ status }) => {
    ...
    if (status === 'CANCELLED') return (
        <Box bg="red.500" color="white" p={4} borderRadius="xl" mb={8} textAlign="center" fontWeight="bold">
            ĐƠN HÀNG ĐÃ BỊ HỦY
        </Box>
    );
                ...
                {steps.map((step, index) => (
                    <Step key={index}>
                        <StepIndicator>
                            <StepStatus
                                complete={<StepIcon />}
                                incomplete={<Box w="2" h="2" bg="gray.500" borderRadius="full" />}
...
const OrderDetailPage = () => {
    ...
    if (loading) return <Flex justify="center" h="100vh" align="center" bg={pageBg}><Spinner color="blue.500" /></Flex>;
    if (!order) return <Box p={10}>Đơn hàng không tồn tại</Box>;

    return (
        <Box bg={pageBg} minH="100vh" py={10} color={textColor}>
            <Container maxW="container.xl">
                <Button as={Link} to="/user/orders" leftIcon={<ArrowBackIcon />} mb={6} variant="ghost" color="gray.500">
                    Quay lại danh sách
                </Button>

                <Box bg={cardBg} p={8} borderRadius="2xl" border="1px solid" borderColor={borderColor} mb={8} shadow="lg">
                    <Flex justify="space-between" align="center" wrap="wrap" gap={4} mb={6}>
                        <Box>
                            <Heading size="lg" mb={1}>Đơn hàng #{order.orderCode}</Heading>
                            <Text color="gray.500" fontSize="sm">Ngày đặt: {new Date(order.createdAt).toLocaleString('vi-VN')}</Text>
                        </Box>
                        <Badge 
                            fontSize="md" px={4} py={2} borderRadius="md"
                ...
                <CancelOrderModal isOpen={isOpen} onClose={onClose} onConfirm={handleCancelOrder} />
            </Container>
        </Box>
    );
};
